...
// Check if latitude and longitude are set in the POST data
if (isset($_POST['latitude']) && isset($_POST['longitude'])) {
    $lat = $_POST['latitude'];
    $lon = $_POST['longitude'];
} else {
    echo "Latitude and longitude are not set in the POST data.";
    return;
}
...
$queryLastId = "SELECT `idArticle` FROM `article` ORDER BY `idArticle` DESC LIMIT 1";
$resultLastId = mysqli_query($GLOBALS['ligacao'], $queryLastId);
$lastIdData = mysqli_fetch_array($resultLastId);
$lastId = $lastIdData['idArticle'] + 1;
mysqli_free_result($resultLastId);

// Start the transaction
mysqli_begin_transaction($GLOBALS['ligacao']);

$query1 =
    "INSERT INTO `$dataBaseName`.`article-images`" .
    "(`idArticle`, `fileName`, `mimeFileName`, `typeFileName`, `imageFileName`, `imageMimeFileName`, `imageTypeFileName`, `thumbFileName`, `thumbMimeFileName`, `thumbTypeFileName`, `caption`) values " .
    "('$lastId', '$fileName', '$mimeFileName', '$typeFileName', '$imageFileName', '$imageMimeFileName', '$imageTypeFileName', '$thumbFileName', '$thumbMimeFileName', '$thumbTypeFileName', '$caption')";

$query2 = "INSERT INTO `$dataBaseName`.`article`" .
    "(`idArticle`, `title`, `content`, `date`, `name`, `idCategory`, `latitude`, `longitude`, `protected` ) VALUES " .
    "('$lastId', '$title', '$content', CURDATE(), '$name', '$category', '$latitude', '$longitude', '$protected')";
